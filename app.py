...
def verify_user(student_id, password):
    ...
    try:
        # 필요한 필드만 선택하여 성능 개선
        response = supabase.table('users').select('student_id').eq('student_id', student_id).eq('password', password).limit(1).execute()
...
# 사용자 전체 기록 조회 (정답/오답 모두)
def get_all_user_records(student_id):
    if not supabase:
        return []
    try:
        # answers 테이블에서 조회 시도
        response = supabase.table('answers').select('*').eq('student_id', student_id).order('created_at', desc=True).execute()
        if response.data:
            return response.data
    except:
        pass
    
    # answers 테이블이 없으면 wrong_answers 테이블 사용
    return get_user_records(student_id)
...
def generate_streaming_explanation(question, user_answer, correct_answer):
    ...
    try:
        ...
        # 빈칸 채우기 문제인 경우 특별 안내 추가
        special_note = ""
        if question_type == 'fill_blank':
            special_note = "\n⚠️ 중요: 이 문제는 빈칸 채우기 문제입니다. 학생의 답은 빈칸에 들어갈 단어만 입력한 것이며, 전체 문장이 아닙니다. 빈칸 부분만 체크하여 해설해주세요.\n"
...
# 문제 답변 저장 (정답/오답 모두)
def save_answer(student_id, question, user_answer, correct_answer, is_correct, ai_explanation=""):
    if not supabase:
        return False
    ...
    try:
        ...
        supabase.table('answers').insert(data).execute()
        return True
    except Exception as e:
        error_msg = str(e)
...
def show_statistics(student_id):
    ...
    try:
        # 전체 기록 조회 (정답/오답 모두)
        all_records = get_all_user_records(student_id)
        # 오답만 조회
        wrong_records = get_user_records(student_id)
        
        if not all_records and not wrong_records:
            st.info("아직 기록이 없습니다. 문제를 풀어보세요!")
            return
        
        # 전체 기록이 있으면 전체 기록 사용, 없으면 오답 기록 사용
        records = all_records if all_records else wrong_records
        
        # 전체 통계
        # is_correct 필드가 있으면 정답/오답 구분 가능
        has_is_correct = any('is_correct' in r for r in records)
        
        if has_is_correct:
            # answers 테이블 사용 (정답/오답 모두)
            ...
            # wrong_answers 테이블만 사용 (오답만)
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("총 오답 수", len(wrong_records))
            with col2:
                # 최근 7일 오답 수
                try:
                    recent_7days = [r for r in wrong_records if (datetime.now() - datetime.fromisoformat(r['created_at'].replace('Z', '+00:00'))).days < 7]
            ...
            with col4:
                # 오늘 오답 수
                try:
                    today = [r for r in wrong_records if datetime.fromisoformat(r['created_at'].replace('Z', '+00:00')).date() == datetime.now().date()]
        ...
        if has_is_correct:
            ...
            for r in records:
                ...
                if r.get('is_correct', False):
                    ...
                    type_wrong[q_type] = type_wrong.get(q_type, 0) + 1
            
            if type_correct or type_wrong:
                import pandas as pd
        ...
        # 최근 기록
        display_records = wrong_records if not has_is_correct else [r for r in records if not r.get('is_correct', True)]
        st.subheader("최근 오답 기록")
        for i, record in enumerate(display_records[:10]):
            try:
...
# 세션 상태 초기화
    ...
    if question.get('type') == 'multiple_choice':
        ...
        if question.get('sentence'):
            ...
            if question.get('type') == 'fill_blank':
                # 빈칸 채우기 문제: sentence에서 빈칸 부분을 밑줄로 표시
                if sentence and '_______' in sentence:
                    # 빈칸을 시각적으로 표시 (밑줄 길이 조정)
                    # 정답 단어 길이에 맞게 밑줄 표시
                    correct_answer = str(question.get('correct_answer', ''))
                    # 정답이 여러 개일 수 있으므로 첫 번째 정답의 길이 사용
                    if ',' in correct_answer or '/' in correct_answer:
                        first_answer = correct_answer.replace('/', ',').split(',')[0].strip()
                    else:
                        first_answer = correct_answer.strip()
                    # 밑줄 길이를 정답 길이에 맞춤 (최소 5자)
                    underline_length = max(len(first_answer), 5) if first_answer else 5
                    display_sentence = sentence.replace('_______', '_' * underline_length)
    ...
    if not st.session_state[answer_key]:
        ...
        with col1:
            check_button = st.button("✅ 정답 확인", use_container_width=True, type="primary")
        with col2:
            pass_button = st.button("⏭️ 패스", use_container_width=True)
        ...
        if check_button:
            if selected is None or (isinstance(selected, str) and not selected.strip()):
                ...
                if question.get('type') == 'multiple_choice':
                    ...
                    # 빈칸 채우기: 빈칸 부분만 체크 (전체 문장이 아닌)
                    correct_answer = str(question.get('correct_answer', '')).strip().lower()
                    user_answer_lower = user_answer.strip().lower()
                    # 정답이 여러 개일 수 있으므로 쉼표나 슬래시로 구분된 경우도 처리
                    correct_answers = [ans.strip().lower() for ans in correct_answer.replace('/', ',').split(',')]
                    is_correct = user_answer_lower in correct_answers
                else:
                    correct_answer = str(question.get('correct_answer', '')).strip().lower()
                ...
                # 정답/오답 실시간 저장
                save_answer(student_id, question, user_answer, question.get('correct_answer', ''), is_correct, full_explanation)
                
                # 틀린 문제도 저장 (하위 호환성)
                if not is_correct:
                    save_wrong_answer(student_id, question, user_answer, question.get('correct_answer', ''), full_explanation)
            ...
            # 패스 버튼: AI 응답 없이 바로 다음 문제로
            st.info("⏭️ 문제를 패스했습니다.")
            
            # 패스한 문제도 저장 (오답으로 처리)
            user_answer = "패스"
            save_answer(student_id, question, user_answer, question.get('correct_answer', ''), False, "")
